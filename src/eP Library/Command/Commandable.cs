...
{
    /// <summary>
    /// Represents an instance which can be controlled by command. 
    /// </summary>
    public abstract class Commandable
    {
        ...
        protected IReadOnlyDictionary<string, CommandExecuteInfo> RegisteredCommands => _commandExecInfo;

        protected IReadOnlyDictionary<string, CommandAutoCompleteInfo> RegisteredCommandAutoCompletes => _commandAutoComplInfo;
        ...
        protected Commandable()
        {
            ...
            foreach(MethodInfo method in methods)
            {
                ...
                if (commandAttribute is not null)
                {
                    ...
                    if (!_commandExecInfo.TryAdd(commandAttribute.CommandName, execInfo))
                        throw new Exception("");
                }
                else if(commandAutoComplAttribute is not null)
                {
                    ...
                    if (!_commandAutoComplInfo.TryAdd(commandAutoComplAttribute.CommandName, commandAutoCompleteInfo))
                        throw new Exception("");
                }
            }
        }
        ...
        public string[] AutoCompleteCommand(string command, int index)
        {
            ...
            else
            {
                ...
                foreach(string commandName in RegisteredCommands.Keys)
        ...
        internal void HotReload()
        {
            ...
            foreach(MethodInfo method in methods)
            {
                ...
                if (commandAttribute is not null)
                {
                    CommandFunction commandFunc = method.CreateDelegate<CommandFunction>(this);
                    CommandExecuteInfo execInfo = new CommandExecuteInfo(commandAttribute.CommandName, commandAttribute.CommandDescription, commandFunc);
                    _commandExecInfo[commandAttribute.CommandName] = execInfo;
                }
                else if(commandAutoComplAttribute is not null)
                {
                    CommandAutoCompleteFunc commandAutoCompleteFunc = method.CreateDelegate<CommandAutoCompleteFunc>(this);
                    CommandAutoCompleteInfo commandAutoCompleteInfo = new CommandAutoCompleteInfo(commandAutoComplAttribute.CommandName, commandAutoCompleteFunc);
                    _commandAutoComplInfo[commandAutoComplAttribute.CommandName] = commandAutoCompleteInfo;
                }
            }
        }
    }
