...
public class RelementShader: VeldridShader
{
    private RelementShader(ShaderSetDescription shaderSetDesc, ResourceLayout shaderResLayout) : base(shaderSetDesc, shaderResLayout)
    {
    ...
    public static RelementShader CreateRelementShader(GraphicsDevice renderDevice)
    {
        ResourceFactory resourceFactory = renderDevice.ResourceFactory;
        
        byte[]? vertexShader = AssemblyResourceManager.GetEmbeddedResource($"KartLibrary.Engine.Render.Shaders.Relement.vertexShader.glsl");
        byte[]? fragmentShader = AssemblyResourceManager.GetEmbeddedResource($"KartLibrary.Engine.Render.Shaders.Relement.fragmentShader.glsl");
        if (vertexShader is null || fragmentShader is null)
            throw new Exception($"Cannot create relement shader!");
        ...
        return new RelementShader(shaderSetDescription, shaderResourceLayout);
    }
}