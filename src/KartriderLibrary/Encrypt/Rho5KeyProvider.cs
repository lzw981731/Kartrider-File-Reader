...
namespace KartLibrary.Encrypt
{
    public class Rho5KeyProvider
    {
        ...
        public int curSubPos = 0;

        private static uint[,] const_1 = {
            {
...
00FFF030 5
        ...
        public void Refresh()
        {
            ...
            for(int i = 0; i < 16; i++)
            {
                int index_5 = ((0x40 + 0x3C - (i << 2)) & 0x3F) >> 2;
                int index_4 = ((0x40 + 0x10 - (i << 2)) & 0x3F) >> 2;
                int index_l = index_5;
                int index_c = ((0x40 + 0x34 - (i << 2)) & 0x3F) >> 2;
                int index_r = index_4;
                int index_a = 0x4C >> 2;
                int index_b = ((0x40 + 0x28 - (i << 2)) & 0x3F) >> 2;
                int index_03 = 0x48 >> 2;
                int index_oth = ((0x40 + 0x38 - (i << 2)) & 0x3F) >> 2;
                int index_s54 = index_5;
                int index_sab = 0x48 >> 2;
                int index_s03 = 0x4C >> 2;
                setFourVar(
                    ref var_0[index_s54],
                    ref var_0[index_sab],
                    ref var_0[index_s03],
        ...
        private unsafe byte[] getInitFileDataArr(string filename, byte[] key, int u2)
        {
            return null;
        ...
        {
            uint firstRound = getConst03(numFs);
            uint secondRound = getConst(5, setVar) ^ getConst(4, numL);
        ...
        {
            uint tmpSvar54 = getConst(5, var5) ^ getConst(4, var4) ^ (varL << 8) ^ (varC) ^ (varR >> 8);
            uint tmpSvarAb = varA + varB;
            uint tmpSvar03 = getConst03(var03);
            uint tmpSvarn = (tmpSvar54 + tmpSvarAb) ^ tmpSvar03 ^ varOth;
            svar54 = tmpSvar54;
            svarab = tmpSvarAb;
            svar03 = tmpSvar03;
            svarn = tmpSvarn;
