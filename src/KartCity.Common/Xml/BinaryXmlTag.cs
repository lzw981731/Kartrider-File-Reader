...
namespace KartCity.Common.Xml
{
    public class BinaryXmlTag: DynamicObject
    {
        ...
        public BinaryXmlTag GetAttributeContinue(string attribute, out dynamic? output)
        {
            if (!Attributes.ContainsKey(attribute))
                output = null;
            else
                output = new BinaryXmlAttributeValue(Attributes[attribute]);
            return this;
        }
        ...
        public BinaryXmlTag SetAttributeContinue(string name,string value)
        {
            if (!Attributes.ContainsKey(name))
                _attributes.Add(name, value);
            else
        ...
        public void Add(BinaryXmlTag childTag)
        {
            Children.Add(childTag);
        }
        
        public BinaryXmlTag AddContinue(BinaryXmlTag childTag)
        {
            Children.Add(childTag);
            return this;
        }
        ...
        public byte[] ToBinary(Encoding encoding)
        {
            using (MemoryStream ms = new MemoryStream())
            {
                ...
                //TagName
                bw.WriteKRString(encoding, Name);
                //Text
                bw.WriteKRString(encoding, Text);
                //Attributes
                bw.Write(Attributes.Count);
                foreach (KeyValuePair<string, string> key in Attributes)
                    bw.WriteKRStringPair(encoding, key.Key, key.Value);
                //SubTags
        ...
        public void ToString(ref TextFormater formater,int nowLevel)
        {
            ...
            if(HaveAttributes)
            {
                ...
                foreach(KeyValuePair<string,string> KeyPair in Attributes)
                {
                    ...
                    attFormat.Add($"{KeyPair.Key}=\"{SecurityElement.Escape(KeyPair.Value)}\"");
                }
